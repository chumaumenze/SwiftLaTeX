...
function _allocate(content) {
  ...
  Module.HEAPU8.set(new Uint8Array(content), res);
  return res;
}
...
Module.onRuntimeInitialized = function () {
  ...
  Module.compileXetex = function () {
    const target =
      "/" + Module.mainFile.substring(0, Module.mainFile.length - 4) + ".xdv";
  ...
  Module.compileDviPDF = function () {
    const target =
      "/" + Module.mainFile.substring(0, Module.mainFile.length - 4) + ".pdf";
...
//       case "fsWrite":
//         self.postMessage({
//           ...Module.writeFile(data.filePath, data.content),
//           cmd,
//         });
//         break;
//       case "setMainFile":
